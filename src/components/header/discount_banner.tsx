...
export default function DiscountBanner() {
  ...
  return (
    ...
    >
      <Link
        href={config.url}
        className={`alert flex w-full justify-center rounded-none border-none p-2 text-center text-xs shadow-none transition-colors duration-1000 ${
          isAccented ? "bg-base-200" : "bg-base-300"
        }`}
      >
        <div className="leading-relaxed [text-wrap:balance]">
          <span className="text-base-content">
            {currentEmoji} <span className="font-semibold">{config.title}</span>{" "}
            – Use code{" "}
            <code
              className={`font-mono tracking-wide underline underline-offset-4 transition-all duration-1000 ${
                isAccented
            ...
            to get <span className="font-semibold">{config.percentage}%</span>{" "}
            discount.
            <span className="inline-block w-2" />
            <span
              className={`rounded-full border border-dashed px-2 py-1 transition-all duration-1000 ${
                isAccented
            ...
            >
              ...
              <span
                className={`font-semibold transition-colors duration-1000 ${
                  hours < 2 && isAccented ? "text-warning/90" : ""
                }`}
              >
                remaining
              </span>
